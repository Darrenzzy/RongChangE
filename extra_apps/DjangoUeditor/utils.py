...
def fetch_static(url: str, replace_key: str = None):
    ...
    if url_obj.scheme == 'file':
        policy = {"scope": settings.QINIU_BUCKET_NAME}
        img_file = Path(url_obj.path[1:])
        key = f'photos/{int(time.time() * 1000)}/{img_file.name}'
        token = q.upload_token(settings.QINIU_BUCKET_NAME, key=key, policy=policy)
        ret, info = put_file(token, key, img_file)
    else:
        bucket = BucketManager(q)
        key = url.replace(f"{url_obj.scheme}://{url_obj.netloc}/", "") if replace_key is None else replace_key
        ret, info = bucket.fetch(url, settings.QINIU_BUCKET_NAME, key)

    # print(f"qi-niu fetch static - {url} to bucket manager result: ret={ret}, info={info}")

    # if info.status_code != 200:
    #     print(f"info = {info.status_code}, {info}, url={url}")
        # assert ret['key'] == key

    return (True, f"{settings.QINIU_DOMAIN}{key}", ret.get("size", 0)) if info.status_code == 200 and ret['key'] == key else (False, url, 0)
