...
from qiniuupload.qiniuupload import get_token
from .utils import fetch_static
...
# 保存上传的文件
def save_upload_file_with_qiniu(fileObject, key):
    """
    fileObject: 上传的文件
    """
    token = get_token(key=key)
    put_stream(token, key, fileObject.file, fileObject.name, fileObject.size)
    return u'SUCCESS'
...
def UploadFile(request):
    ...
    if action in upload_allow_type:
        allow_type = list(request.GET.get(
            upload_allow_type[action], USettings.UEditorUploadSettings.get(upload_allow_type[action], "")))
    ...
    if state == "SUCCESS":
        if action == "uploadscrawl":
            ...
            if upload_module_name:
                ...
                if settings.UEDITOR_USE_QINIU:
                    key = f'photos/{timestamp}/{file.name}'
                    state = save_upload_file_with_qiniu(file, key)
                else:
                    state = save_upload_file(file, OutputPath)

    # 返回数据
    if settings.UEDITOR_USE_QINIU:
        url = urljoin(settings.QINIU_DOMAIN, f'photos/{timestamp}/{file.name}')
    else:
...
def catcher_remote_image(request):
    ...
    for remote_url in remote_urls:
        ...
        if remote_original_ext in allow_type:
            ...
            # 读取远程图片文件
            fetch_result, qi_path_format, _qi_size = fetch_static(url=remote_url, replace_key=o_filename)
            state = "SUCCESS" if fetch_result else '七牛云抓取图片错误'
...
def get_output_path(request, path_format, path_format_var):
    ...
    if not USettings.UEditorSettings.get('is_qiniu', False):
        # 非七牛云时，才创建文件目录； 七牛云时，不创建目录
