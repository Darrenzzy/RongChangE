...
class UEditorWidget(forms.Textarea):
    ...
    def __init__(self, attrs=None):
        ...
        if toolbars == "full":
            ...
            self.ueditor_settings["toolbars"] = USettings.UEditorSettings['toolbars']
            ...
            if not self.ueditor_settings['toolbars']:
                del self.ueditor_settings['toolbars']       # settings中toolbars=[]时，默认为全部
...
class ImagePreviewWidget(forms.URLInput):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        if context["widget"]['attrs'].get("required") != context['widget'].get("required"):
            context["widget"]['attrs']['required'] = context['widget']["required"]
        return mark_safe(render_to_string('preview.html', context))
