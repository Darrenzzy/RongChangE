...
def register(models, **kwargs):
    ...
    def _model_admin_wrapper(admin_class):
        site.register(models, admin_class, **kwargs)
    return _model_admin_wrapper


def register_plugin(view):
    def _view_admin_wrapper(plugin_class):
        site.register_plugin(plugin_class, view)
    return _view_admin_wrapper


def register_view(path, name):
    def _view_class_wrapper(view_class):
        site.register_view(path, view_class, name)
    return _view_class_wrapper


def admin_order_field(ordering: str):
    """ 为方法添加 admin_order_field 字段 """

    def wrapper_func(func):
        func = getattr(func, "original", func)
        func.admin_order_field = ordering

        @wraps(func)
        def wrapper_kwargs(*args, **kwargs):
            return func(*args, **kwargs)

        wrapper_kwargs.original = func
...
def allow_export(value: bool):
    """ 为方法添加 allow_export 字段 """

    def wrapper_func(func):
        func = getattr(func, "original", func)
        func.allow_export = value
