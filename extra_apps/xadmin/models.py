...
def add_view_permissions(sender, **kwargs):
    ...
    permission_map = {
        f"{x['content_type_id']}_{x['codename']}": True
        for x in Permission.objects.values("content_type_id", "codename")
    }
    # for each of our content types
    for content_type in ContentType.objects.all():
        # build our permission slug
        for per in ["view", "export"]:
            codename = "%s_%s" % (per, content_type.model)

            # if it doesn't exist ...
            # if not Permission.objects.filter(content_type=content_type, codename=codename).exists():
            if not permission_map.get(f"{content_type.pk}_{codename}"):
                # add it
                Permission.objects.create(
                    content_type=content_type, codename=codename, name="Can %s %s" % (per, content_type.name)
                )
...
reversion.register(get_user_model())
...
class Bookmark(models.Model):
    ...
    url_name = models.CharField(_(u'Url Name'), max_length=64, db_index=True)
...
class Log(models.Model):
    ...
    ip_addr = models.CharField(_('action ip'), blank=True, null=True, max_length=1000)
    ...
    data = models.TextField(verbose_name="数据记录", null=True, blank=True)
