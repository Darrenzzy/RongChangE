...
def get_model_fields(
        obj: typing.Union[models.Model, models.base.ModelBase, models.query.QuerySet]
) -> typing.Dict:
    """
    根据 app 名称 + model 名称 获取到 model 对象

    :param obj: app 名称
    :return: {'company': '所属公司', 'wechat': '所属微信公众号', ... }
    """
...
def get_obj_attr(
        model_instance_object: typing.Union[models.Model, models.base.ModelBase, models.query.QuerySet, typing.List],
        recursive_depth: int = 0
) -> typing.Union[typing.Dict, typing.List]:
    """
    根据一个 model instance + app name + model name + exclude fields list
    应用最好的地方就是 copy 一个根据id model instance 所需要的字段内容

    # 参考 MedChat/apps/interaction/tests.py
    :param model_instance_object: model instance
    :param recursive_depth:        递归深度数限制，默认0，一直累加, 一直达到25层时直接返回，最大层数是25
    :return: 该 model instance 除exclude fields list 之外的所有字段内容 => {'company': <Company: Yuemia>, ...}
    """
    if not isinstance(model_instance_object, (models.Model, models.base.ModelBase, models.query.QuerySet)):
        return model_instance_object

    recursive_depth += 1

    fields_dict = get_model_fields(model_instance_object)
    attr_dict = {}
    for x in fields_dict:
        if recursive_depth >= 3:
            return attr_dict

        result = getattr(model_instance_object, x)
        # attr_dict[x] = getattr(result, "id", result) if instance_type else result
        attr_dict[x] = get_obj_attr(result, recursive_depth) if isinstance(result, (
            models.Model, models.base.ModelBase, models.query.QuerySet)) else f"{result}"

    return attr_dict


def extract_map(data: typing.Dict) -> typing.Dict:
    rets = {}
    for k, v in data.items():
        if isinstance(v, (models.Model, models.base.ModelBase)):
            rets[k] = get_obj_attr(v)
        elif isinstance(v, (models.query.QuerySet, list)):
            rets[k] = [get_obj_attr(vp) for vp in v]
        else:
            rets[k] = f"{v}"
...
def create_event_log(
        ...
        app_label: str = None,
        model_name: str = None,
        obj: typing.Union[models.Model, models.base.ModelBase] = None,
        object_repr: str = None,
        original_data: typing.Union[typing.Dict, None] = None,
        new_data: typing.Union[typing.Dict, None] = None,
):
    """
    创建 x-admin Log
    ...
    if app_label and model_name:
        try:
            log.content_type = ContentType.objects.get(app_label=app_label, model=model_name)
        except Exception:
            pass

        if not obj and object_repr:
