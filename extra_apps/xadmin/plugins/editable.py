...
class EditPatchView(ModelFormAdminView, ListAdminView):
    ...
    def get(self, request, object_id):
        ...
        fields = [f for f in request.GET.get('fields', '').split(',') if f in model_fields]
        defaults = {
            "form": self.form,
            "fields": fields,
        ...
        s = '{% load i18n crispy_forms_tags %}<form autocomplete="off" method="post" action="{{action_url}}">{% crispy form %}' + \
        ...
        c = template.RequestContext(
            request, {'form': form, 'action_url': self.model_admin_url('patch', self.org_obj.pk)}
        )

        return HttpResponse(t.render(c))

    @filter_hook
    def after_patch_form_save(self, request):
        pass
