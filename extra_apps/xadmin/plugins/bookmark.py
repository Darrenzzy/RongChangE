...
class BookmarkAdmin(object):
    ...
    def queryset(self):
        if self.user.is_superuser:
            return Bookmark.objects.all().select_related("user", "content_type")
        return Bookmark.objects.filter(Q(user=self.user) | Q(is_share=True)).select_related("user", "content_type")
