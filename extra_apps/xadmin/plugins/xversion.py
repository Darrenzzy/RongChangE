...
def register_models(admin_site=None):
    ...
    for model, admin in admin_site._registry.items():
        if getattr(admin, 'reversion_enable', True):
...
class BaseReversionView(ModelAdminView):
    ...
    def init_request(self, *args, **kwargs):
        ...
        # self.history_latest_first = bool(self.user.is_superuser)
    ...
    def get_context(self):
        ...
        action_list = [
            ...
            in self._reversion_order_version_queryset(Version.objects.get_for_object_reference(
                self.model,
                self.obj.pk,
            ).select_related("revision__user", "content_type"))
        ]
        context.update({
    ...
    def get_version_object(self, version):
        ...
        for field_name, pks in obj_version.m2m_data.items():
            ...
            if f.remote_field and isinstance(f.remote_field, models.ManyToManyRel):
                pks__all = f.remote_field.through._default_manager.get_queryset().filter(pk__in=pks).all()
                getattr(obj, f.name).set(list(pks__all.values_list("pk", flat=True)))
        ...
        for field in self.fields:
            html += (
                    ...
                    (_('Current: %s') % self.attrs.pop('orgdata', ''),
                     render_field(field, form, context, template_pack=template_pack, attrs=self.attrs)))
        return html
...
class InlineDiffField(Field):
    ...
    def render(self, form, context, template_pack=TEMPLATE_PACK, **kwargs):
        ...
        if not instance.pk:
            return super(InlineDiffField, self).render(form, context)
...
class ReversionAdmin(object):
    ...
    list_display = ("id", '__str__', 'date_created', 'user', 'comment')
    # list_display_links = ('__str__', )

    list_filter = ('date_created', 'user', 'revisions__content_type')
    ...
    def queryset(self):
        q = super().queryset()
        return q.select_related('user', ).prefetch_related("revisions", )
