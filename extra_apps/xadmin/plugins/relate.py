...
class BaseRelateDisplayPlugin(BaseAdminPlugin):
    ...
    def _get_relate_params(self):
        return escape(RELATE_PREFIX + self.relate_obj.lookup), escape(self.relate_obj.value)

    def _get_input(self):
        return '<input autocomplete=\"off\" type="hidden" name="%s" value="%s" />' % self._get_relate_params()
