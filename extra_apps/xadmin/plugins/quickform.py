...
class QuickFormPlugin(BaseAdminPlugin):
    def init_request(self, *args, **kwargs):
        if (
            ...
            and self.request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest'
...
class QuickAddBtnPlugin(BaseAdminPlugin):
    remove_rel_add_permission = []

    def formfield_for_dbfield(self, formfield, db_field, **kwargs):
        # 详情页移除外键添加权限
        rel_add_enable = True
        if db_field and db_field.name in self.remove_rel_add_permission:
            rel_add_enable = False
