...
class CharFieldImagePreviewWidget(forms.URLInput):
    """
    字符串字段图片预览
    """
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        value_slice = value.split(",") if value else []
        context = self.get_context(name, value, attrs)
        context["widget"]["value_slice"] = value_slice
        if context["widget"]['value'] is None:
            context["widget"]['value'] = ''
        return mark_safe(render_to_string('xadmin/widgets/preview.html', context))


class CharFieldImagePreviewPlugin(BaseAdminPlugin):
    """
    CharField 图片url预览功能，支持多个图片url逗号拼接，最大显示12个图片，成九宫格展示
        - style_fields = {"xxx": "preview"}
    """

    def init_request(self, *args, **kwargs):
        return hasattr(self.admin_view, 'style_fields') and 'preview' in self.admin_view.style_fields.values()

    def get_field_style(self, attrs, db_field, style, **kwargs):
        if style == 'preview':
            return {'widget': CharFieldImagePreviewWidget}
        return attrs


site.register_plugin(CharFieldImagePreviewPlugin, DetailAdminView)
site.register_plugin(CharFieldImagePreviewPlugin, ModelFormAdminView)
