...
from django.db.models import BooleanField, NullBooleanField, JSONField
...
class ExportPlugin(BaseAdminPlugin):
    ...
    def get_xlsx_export(self, context):
        ...
        for sheet_index in range(sheet_num):
            sheet = book.add_worksheet(
                u"%s %s %s" % (_(u'Sheet'), force_str(model_name), sheet_index)
            )
            new_data = datas[sheet_index * 65534: (sheet_index + 1) * 65534]
            for rowx, row in enumerate(new_data):
    ...
    def get_xls_export(self, context):
        ...
        # # xlwt 最多处理 65536 数据量
        # if len(datas) >= 65536:
        #     raise Exception('数据量过大，超过65536行处理不了')
    ...
    def get_response(self, response, context, *args, **kwargs):
        ...
        response = HttpResponse(
            content_type="%s; charset=gb18030" % self.export_mimes[file_type]
        )

        file_name = self.opts.verbose_name.replace(' ', '_')
        response['Content-Disposition'] = ('attachment; filename=%s.%s' % (
            f'{file_name}_{datetime.datetime.now().strftime("%Y%m%d%H%M")}', file_type
        )).encode('utf-8')

        response.write(getattr(self, 'get_%s_export' % file_type)(context))
        self.log(
            flag="export",
            message="导出 [%s] %s-格式, 共计%s条数据;" % (file_name, file_type, len(context['results'])),
