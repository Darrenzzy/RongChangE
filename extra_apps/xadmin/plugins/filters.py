...
class FilterPlugin(BaseAdminPlugin):
    ...
    # ase_client 必须实现  加密方法 encrypt(), decrypt()
    aes_client = []
    aes_search_fields = ()
    ...
    def get_list_queryset(self, queryset):
        ...
        if self.list_filter:
            for list_filter in self.list_filter:
                if callable(list_filter):
                    ...
                    # django 4.2 后 lookup_needs_distinct -> lookup_spawns_duplicates
                ...
                if spec and spec.has_output():
                    ...
                    # except ValidationError as e:
                        ...
                        self.admin_view.message_user(_("<b>Filtering error:</b> %s") % str(e.args), 'error')
        ...
        try:
            ...
            if isinstance(queryset, models.query.QuerySet) and lookup_params:
                ...
                for k, v in lookup_params.items():
                    list_v = v.split(',') if isinstance(v, str) else [v]
                    list_v_len = len(list_v)
                    if list_v_len > 0:
                        new_lookup_parames.update({k: list_v[0] if list_v_len == 1 else list_v})
                    else:
        ...
        if self.search_fields and query:
            ...
            if self.aes_search_fields:
                aes_search_orm_lookups = [
                    construct_search(str(aes_search_field)) for aes_search_field in self.aes_search_fields
                ]
            ...
            for bit in query.split():
                ...
                if self.aes_client and not isinstance(self.aes_client, (list, tuple)):
                    self.aes_client = [self.aes_client, ]

                aes_client_len = len(self.aes_client)
                if self.aes_search_fields and \
                        0 < aes_client_len == len([True for x in self.aes_client if hasattr(x, "encrypt")]):
                    for _aes_client in self.aes_client:
                        aes_or_queries = [
                            models.Q(**{aes_orm_lookup: _aes_client.encrypt(bit)})
                            for aes_orm_lookup in aes_search_orm_lookups
                        ]
                        or_queries.extend(aes_or_queries)
