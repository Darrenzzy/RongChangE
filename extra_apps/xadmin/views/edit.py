...
from xadmin.util import unquote, extract_map
...
class ModelFormAdminView(ModelAdminView):
    ...
    def change_message(self):
        change_message, new_data, origin_data = [], None, None
        # print(f"self.form_obj.initial={self.form_obj.initial}")
        # print(f"self.form_obj.initial={type(self.form_obj.initial['tag'])}")
        # print(f"self.form_obj.cleaned_data={self.form_obj.cleaned_data}")
        # print(f"self.form_obj.changed_data={self.form_obj.changed_data}")
        if self.org_obj is None:
            change_message.append(_('新增 %s 信息.') % (
                    get_text_list(self.form_obj.changed_data, _('and'))
            ))
            new_data = extract_map(self.form_obj.cleaned_data)

        elif self.form_obj.changed_data:
            # 初始值       self.form_obj.initial
            # 新 值       self.form_obj.cleaned_data
            # 变化字段      self.form_obj.changed_data
            # change_message.append(_('Changed %s.') % get_text_list(self.form_obj.changed_data, _('and')))
            change_message.append(_('Changed %s.') % (
                    get_text_list(self.form_obj.changed_data, _('and')) + ' 信息'
            ))
            origin_data = extract_map(self.form_obj.initial)
            new_data = extract_map(self.form_obj.cleaned_data)

        change_message = ' '.join(change_message)
        return change_message or _('No fields changed.'), new_data, origin_data

    @filter_hook
    def save_models(self):
        self.new_obj.save()
        flag = self.org_obj is None and 'create' or 'change'
        message, new_data, origin_data = self.change_message()
        self.log(flag, message, self.new_obj, original_data=origin_data, new_data=new_data)
