...
class WidgetTypeSelect(forms.Widget):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        output.append(u'</ul>')
        output.append('<input autocomplete="off" type="hidden" id="%s_input" name="%s" value="%s"/>' %
                      (final_attrs['id'], escape(name), escape(force_text(value))))
        return mark_safe(u'\n'.join(output))
...
class QuickBtnWidget(BaseWidget):
    ...
    btns = forms.CharField(label=_('小组件配置'), required=False, widget=exwidgets.AdminTextInputWidget)

    def convert(self, data):
        self.btns = data.get('btns')
        if not self.btns:
            self.btns = []
        if self.btns and type(self.btns) == str and self.request.method == 'POST':
            self.btns = self.btns.replace('\'', '"')
            self.btns = json.loads(self.btns)
            data['btns'] = self.btns
    ...
    def context(self, context):
        ...
        for b in self.btns:
            ...
            if 'model' in b:
                ...
                try:
                    btn['url'] = reverse(b.get('url', None))
            ...
            if 'icon' in b:
                btn['icon'] = b['icon']
            btn['type'] = b.get('type', 'link')     # link, upload
            if 'settings' in b:
                settings = dict()
                settings['title'] = b['settings'].get('title', '导入文件')
                try:
                    settings['url'] = reverse(b['settings'].get('url', None))
                except NoReverseMatch:
                    settings['url'] = b['settings'].get('url', '')
