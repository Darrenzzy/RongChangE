...
from xadmin.util import static, json, vendor, sortkeypicker, create_event_log
...
class BaseAdminObject(object):
    ...
    def get_form_params(self, new_params=None, remove=None):
        ...
        return mark_safe(''.join(
            '<input autocomplete="off" type="hidden" name="%s" value="%s"/>' % (escape(k), escape(v)) for k, v in p.items() if v))
    ...
    def log(
        ...
        try:
            ip_addr = self.request.META['HTTP_X_FORWARDED_FOR'] if self.request.META['HTTP_X_FORWARDED_FOR'] else \
                self.request.META['REMOTE_ADDR']
        except Exception as e:
            ip_addr = self.request.META['REMOTE_ADDR']

        create_event_log(
            user=self.user, ip_addr=ip_addr, app_label=app_label, flag=flag, message=message,
            model_name=model_name, obj=obj, object_repr=object_repr, original_data=original_data, new_data=new_data
        )
...
class BaseAdminPlugin(BaseAdminObject):
    ...
    def has_export_permission(self, request=None, obj=None):
        if not self.user:
            return False
    ...
    def get_nav_menu(self):
        ...
        for model, model_admin in self.admin_site._registry.items():
            ...
            if app_key in nav_menu:
                ...
                # 增加apps排序字段
                app_order = self.apps_label_order.get(app_label.lower(), 0)
                app_order = -app_order
        ...
        nav_menu.sort(key=sortkeypicker(["order", "title"]))
...
class ModelAdminView(CommAdminView):
    ...
    def has_export_permission(self, request=None, obj=None):
        codename = get_permission_codename('export', self.opts)
        return ('export' not in self.remove_permissions) and self.user.has_perm('%s.%s' % (self.app_label, codename))
